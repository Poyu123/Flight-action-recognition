...
output_dir=os.path.join(output_dir,'筋斗识别'+datetime.now().strftime("%Y_%m_%d_%H_%M_%S"))
'''
    ...
    while(1):
        ...
        if abs(sum_pitch-360) <= vary_num :
            ...
            for i in range(0,len(deltaR)) :
                #print(pitchdelta(pitch[counter],pitch[counter+1]))
            ...
            if variance <= variance_num :
                ...
                printphoto(pitch,time,os.path.join(output_dir,'photo'),trigger_counter,(counter+1),ID)

        trigger_counter=counter

    print("以完成对飞机号为"+str(ID)+"的筋斗飞行动作的识别")
