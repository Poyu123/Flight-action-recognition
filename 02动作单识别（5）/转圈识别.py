...
output_dir=os.path.join(output_dir,os.path.basename(os.path.dirname(source_dir))[:3]+'_转圈识别'+datetime.now().strftime("%Y_%m_%d_%H_%M_%S"))
...
output_dir=os.path.join(output_dir,'转圈识别'+datetime.now().strftime("%Y_%m_%d_%H_%M_%S"))
'''
    ...
    while(1):
        ...
        if abs(sum_yaw-360) <= vary_num :
            ...
            for i in range(0,len(deltaR)) :
                #print(yawdelta(yaw[counter],yaw[counter+1]))
            ...
            if variance <= variance_num :
                ...
                printphoto(yaw,time,os.path.join(output_dir,'photo'),trigger_counter,(counter+1),ID)

        trigger_counter=counter

    print("以完成对飞机号为"+str(ID)+"的转圈飞行动作的识别")
